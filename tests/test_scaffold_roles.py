...
def test_scaffold_creates_expected_files(tmp_path, monkeypatch):
    monkeypatch.setattr(scaffold_roles, "ROLES_DIR", tmp_path)

    scaffold_roles.scaffold("kiosk")

    assert (tmp_path / "kiosk" / "tasks" / "main.yml").exists()
    assert (tmp_path / "kiosk" / "defaults" / "main.yml").exists()
    assert (tmp_path / "kiosk" / "meta" / "main.yml").exists()


def test_scaffold_does_not_overwrite_existing_files(tmp_path, monkeypatch):
    monkeypatch.setattr(scaffold_roles, "ROLES_DIR", tmp_path)
    tasks = tmp_path / "kiosk" / "tasks" / "main.yml"
...
def test_main_scaffolds_common_and_tags_from_api(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "common" / "tasks" / "main.yml").exists()
    assert (tmp_path / "kiosk" / "tasks" / "main.yml").exists()
